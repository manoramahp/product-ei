...
/**
 * This class tests consuming javax.jms.BytesMessage from a queue using inbound endpoints
 * https://wso2.org/jira/browse/ESBJAVA-5085
 */
public class JMSInboundBytesMessageTypeTestCase extends ESBIntegrationTest{
    ...
    @Test(groups = { "wso2.esb" }, description = "Polling BytesMessage from a Queue")
    public void testBytesMessagesType() throws Exception {
        ...
        String queueName = "BytesMessageQueue";

        try {
            sender.connect(queueName);
            for (int i = 0; i < 3; i++) {
                sender.sendBytesMessage(new String("<?xml version='1.0' encoding='UTF-8'?>" +
    ...
    private OMElement addEndpoint() throws Exception {
        ...
        synapseConfig = AXIOMUtil
                .stringToOM("<inboundEndpoint xmlns=\"http://ws.apache.org/ns/synapse\"\n" +
                        ...
                        "        <parameter name=\"transport.jms.Destination\">BytesMessageQueue</parameter>\n" +
